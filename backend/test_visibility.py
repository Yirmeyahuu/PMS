...
try:
    ...
    if 'patient' in locals():
        ClinicalNote.objects.filter(patient=patient).delete()
        PatientCase.objects.filter(patient=patient).delete()
        Patient.objects.filter(id=patient.id).delete()
    User.objects.filter(email__in=["prac1@test.com", "prac2@test.com"]).delete()
