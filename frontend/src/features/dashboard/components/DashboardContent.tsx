...
export const DashboardContent: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      {/* Header - Fixed */}
      <div
        className="flex-shrink-0 border-b border-gray-200 bg-white/80 backdrop-blur-sm p-3 sm:p-4 md:px-6 md:py-4"
      >
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h1 className="text-xl md:text-2xl font-bold text-gray-900 mb-1 font-heading truncate">
              Hi there, {user?.first_name || 'User'}!
            </h1>
            <p className="text-xs md:text-sm text-gray-500 font-body truncate">
          ...
          {!isLoading && (
            <button
              onClick={refresh}
              className="flex items-center gap-1.5 md:gap-2 px-3 py-1.5 md:px-4 md:py-2 bg-white border border-gray-200 text-steady-slate rounded-2xl hover:bg-clinical-cloud transition-colors text-xs md:text-sm font-medium font-body flex-shrink-0"
            >
      ...
      <div
        className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 pb-6 md:pb-6"
      >
